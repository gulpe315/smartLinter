...
def summarize(records, variant, cases):
    ...
    return {
        "runs": len(group),
        "seeded_error_recall_pct": round(100 * sum(r["verdict"]["seeded_error_caught"] for r in erroneous) / len(erroneous), 2),
...
def main():
    ...
    for repetition in range(1, REPETITIONS + 1):
        for case in cases:
            for variant in ("english", "korean_baseline"):
    ...
    summaries = {variant: summarize(records, variant, cases) for variant in ("english", "korean_baseline")}
    output = {"timestamp": time.strftime("%Y-%m-%dT%H:%M:%S%z"), "model": MODEL, "repetitions": REPETITIONS, "dataset": "english_profile_dataset.json", "warmup": warmup, "instruction_texts": {"english_monolingual": EN_MONOLINGUAL, "english_bilingual": EN_BILINGUAL}, "summaries": summaries, "records": records}
    path = Path(__file__).parent / "english_profile_benchmark_results.json"
