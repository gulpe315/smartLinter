...
1. **1단계(사전 확장) 완료·커밋(`301a5c6`).** 로컬 LLM 언어품질 서베이(조사 호응
   등, `QUESTION_LOCAL_MODEL_LINGUISTIC_QUALITY.md`) 결과 최우선 저위험 항목으로
   외래어/불변 맞춤법 오탈자 사전 확장을 선택 → Codex+agy 독립 스코핑 →
   3곳 불일치(어플리케이션 포함여부/설레임 포함여부/악세사리 표기) 재조율
   라운드 1회 → 남은 사소한 2곳(악세사리 변형개수/어의없다류 포함여부)은 Claude가
   더 보수적인 쪽으로 판단해 정리 → `spelling.loanword`(15개)+`spelling.invariant`
   (8개) 신규 카테고리 구현(순수 JSON+테스트, Rust 로직 변경 없음, `detect()`가
   이미 카테고리를 데이터기반으로 순회) → `cargo test` 69/69 독립 재검증 → 커밋.
   레퍼런스→참조/카테고리→범주 같은 의견성 대체, 바램→바람(색바램 등 도메인
   충돌), 설레임→설렘(롯데웰푸드 실제 등록상표 충돌) 등은 명시적으로 배제/보류
   처리. 서버 재기동 후 라이브검증 예정.
