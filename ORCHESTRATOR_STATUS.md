...
2. **Word Step 2 나머지 완료조건 미검증.** 사이드로딩 자체는 성공했지만
   ...
   조작이 필요해 사용자 참여 필수.
3. **`bridge_client.ts`의 `connect()`가 WS 실패 시 REST로 폴백 안 함
   (부수 버그, Codex 발견, 2026-08-28).** 이번 Word 세션에선 문제로
   드러나진 않았지만(WS가 항상 성공), WS가 막히는 환경에서는 재현 가능.
   순수 코드 수정이라 Word 라이브 없이 착수 가능.
