...
1. **Kiwi 스파이크 Step 2 환경 준비.** 위 참고 — Windows Sandbox 활성화
   또는 기존 VM 소프트웨어 중 사용자가 정할 것. 명시적으로 요청하기 전엔
   착수하지 말 것.
2. **Word 사이드로딩 인프라 미착수.** `plugins/word/manifest.xml`이
   가리키는 `word_taskpane.html` 진입점+dev 서빙 인프라 자체가 없음(TS
   로직만 존재, 확인됨 2026-08-27). InDesign만 실제로 동작 중.
3. **"동일 이슈 일괄 적용" 기능 미구현.** 예전 사용자 요청 4건 중 하나,
   코드베이스에 없음을 확인함(2026-08-27) — `start_batch_scan`류 전체스캔
   진행바와는 다른 기능.

셋 다 사용자가 명시적으로 요청하기 전엔 착수하지 말 것.
