...
**⚠️ 정책 결정(2026-08-31, 사용자 확정, 다음 세션에도 유효) — 실제
Word/InDesign 라이브 검증과 Kiwi VM 테스트는 전부 "모든 개발을 완료한
후" 한 번에 몰아서 진행한다.** 즉 트랙 하나 끝날 때마다 라이브 확인을
권하거나 VM을 준비하자고 제안하지 말 것 — 각 트랙은 mock/유닛테스트
기반으로 완결하고 다음 트랙으로 넘어간다. 이 정책이 바뀌면(사용자가
다시 명시적으로 언급하면) 이 절을 갱신할 것.
...
2. ~~다국어 QA 프로파일 — 영어만 완료~~ — **부정확.** 실제로는
   ...
   미검증 언어 fail-loud(`85eeafc`)까지 설계 문서(`DESIGN_MULTILINGUAL_
   AND_SOURCE_FIELD_FIX.md`)의 phasing 1~4가 전부 구현 완료됨(2026-08-26).
   **영어 콘텐츠는 "완료"가 아니라 두 번 시도 후 "no-ship 최종
   확정"**(`f852b27` 범용 프로필, `83d80af` few-shot 보강 — 둘 다 사전
   합의된 오탐률 기준 미달로 실패, `2e6fddc` 핸드오프가 "no-ship
   final"로 종결 처리). 남은 건 (a) 일본어/중국어 콘텐츠 — 설계문서가
   "지금 범위·일정 잡지 말 것"이라 명시한 부분, (b) 영어 3차 시도 —
   기존 두 접근 다 실패해서 새 전략 없이는 재시도 의미 없음. **둘 다
   실행 준비된 설계가 없고 새 설계 자문부터 시작해야 함** — "설계
...
5. Kiwi Part A/B는 완료. Part C(VM 콜드부트 스파이크) — **이 PC(2026-
   08-31 기준)는 VirtualBox+`win11` VM이 이미 있어 재개 가능**(이전 PC는
   VM이 없어 보류였음). 단 오늘은 사용자가 이 트랙을 후순위로 미루기로
   결정(2026-08-31).
6. 그 외 낮은 우선순위 백로그(동일 스팬 중복 QA 카드, 다중 Word 창 포커스,
   Ollama 모델 소실 UX, 조사쌍 으로/로·과/와 확장) — 미확인, 착수 전
   각각 git log로 재확인 필요(위 ①②처럼 이미 됐을 가능성 배제 못함).

**교훈(2026-08-31):** 이 문서의 "미착수" 표시를 실행 근거로 그대로
믿지 말 것 — 여러 PC가 병렬 작업하는 프로젝트라 로드맵 절이 실제
git 커밋보다 뒤처지는 일이 이미 두 번(①②) 확인됨. **다음 트랙에
착수하기 전에는 반드시 `git log --oneline --all | grep -i <키워드>`로
해당 기능이 이미 구현됐는지부터 확인할 것.**

**우선순위는 사용자가 정한다 — 자동 결정 금지.** 다음 세션 시작 시 위
1~4(또는 6) 중 어느 걸 먼저 할지 사용자에게 먼저 확인할 것.** 아래 이
절을 먼저 읽을 것.

## 워크플로 재확인 (2026-08-31, 새 PC에서 세션 재개)

다른 PC(2026-08-30 세션)에서 만든 커밋까지 `git pull` 완료. 이 PC에서
`codex --version`(0.147.0)과 `codex exec` 최소 프롬프트 왕복을 직접
실행해 **Codex 사용량 한도가 정상 응답함을 실측 확인** — 위 6번 체크리스트
항목대로 사용자에게 확인한 결과, **원래 역할 분담(Codex가 구현, agy가
설계자문+구현 리뷰, Claude가 오케스트레이터+최종 QA)으로 복귀**하기로
확정함. 즉 T6d-2 이전 방식으로 되돌아간 것이고, agy 단독(설계+구현
