...
**⭐⭐⭐⭐⭐⭐ 마지막 업데이트: 2026-08-29 세션 종료 시점(사용자가
"새 세션에서 이어서 작업할 수 있도록 정리해줘"로 명시적 핸드오프
요청) — 트랙 C T4(인라인 태그 보존)가 T4-1/T4-2까지 구현·검증·커밋
완료, **T4-3(XLIFF 직렬화/역직렬화) 지시서는 작성·커밋까지만 되고
Codex에게 아직 전달 안 됨 — 다음 세션은 여기서 바로 이어서 시작할
것.** 로컬이 원격보다 18개 커밋 앞섬(아직 push 안 함,
`smartlinter-defer-remote-push` 메모리 참고 — 전체 작업 마무리
선언 전엔 push하지 말 것). 아래 이 절을 먼저 읽을 것.**

## ⭐ 다음 세션 시작 시 즉시 할 일

1. `TASK_REQUEST_TRANSLATION_MODE_T4_3.md`(이미 작성·커밋됨, 커밋
   `bb051e8`)를 Codex에게 그대로 전달해 구현시킨다 — 새로 설계
   자문할 필요 없음, 스펙은 `RECONCILED_TRANSLATION_MODE_T4.md`에
   이미 확정돼 있다.
...
3. **설계·구현 지시를 Codex/agy에게 보낸 직후엔 완료 알림만 기다리지
   ...
   (`periodic-progress-updates` 메모리 참고). 단, **완료 판정을
   로그 내용의 문자열 매칭("tokens used" 등)으로 하지 말 것** — 이번
   세션에서 코드 주석에 그 문구가 우연히 포함돼 오탐지가 발생했다.
   실제 완료는 항상 background task-notification으로만 확인할 것,
   Monitor는 "몇 줄 늘었는지" 같은 진행 상황 보고 용도로만 쓸 것.
4. T4-3까지 끝나면 트랙 C의 T4(인라인 태그 보존)가 전체 완료된다.
   그 다음은 T6(새 번역 문서 생성) 착수 — 사용자가 이미 확정한 순서
