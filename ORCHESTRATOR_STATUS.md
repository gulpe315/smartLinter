...
5. **밀린 신규 기능 백엔로그 (agy+Codex 검토는 끝났고 구현 착수 전, 우선순위 1→3→2 권장):**
   ...
   - **신규 (2026-08-25, 사용자 제안, 아직 agy/Codex 교차검증 안 함):** [적용]으로 치환 완료된 카드를 목록에서 완전히 지우지 말고 "완료" 상태로 숨겨뒀다가, "완료 보이기" 토글로 다시 꺼내서 확인·재수정할 수 있게 할 것. Task B(paragraphId 기반 문단 조회)가 선행되어야 "완료된 카드"에서 다시 그 문단으로 정확히 돌아가 재수정할 수 있음 — Task B 완료 후 이 제안을 Codex/agy에 공유해 실현가능성·설계 방향 검토할 것.
