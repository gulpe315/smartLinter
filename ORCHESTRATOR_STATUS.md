...
**별도 트랙 — 로컬 LLM 언어품질 개선, 사용자가 "둘 다 순서대로" 선택:**
1. **1단계(사전 확장) 완료·커밋(`301a5c6`, 후속 `40e6718`)·라이브검증 완료.**
   로컬 LLM 언어품질 서베이(조사 호응 등, `QUESTION_LOCAL_MODEL_LINGUISTIC_QUALITY.md`)
   결과 최우선 저위험 항목으로 외래어/불변 맞춤법 오탈자 사전 확장을 선택 →
   Codex+agy 독립 스코핑 → 3곳 불일치(어플리케이션/설레임/악세사리 표기)
   재조율 1회 → 남은 사소한 2곳(악세사리 변형개수/어의없다류 포함여부)은
   Claude가 임의로 정하지 말고 **후보+근거를 그대로 사용자에게 제시해서 직접
   고르게 함**([[feedback_present_candidates_not_forced_consensus]] 신규 원칙,
   Claude가 처음엔 혼자 "보수적인 쪽"으로 결정해 구현했다가 사용자 지적으로
   재확인 절차를 거침) → 사용자 결정: 악세사리 범위는 유지, 어의없다류는 추가
   → `spelling.loanword`(15개)+`spelling.invariant`(11개, 어의없다류 포함)
   신규 카테고리 구현(순수 JSON+테스트, Rust 로직 변경 없음) → `cargo test`
   69/69 독립 재검증 → 커밋. 레퍼런스→참조/카테고리→범주 같은 의견성 대체,
   바램→바람(색바램 등 도메인 충돌), 설레임→설렘(롯데웰푸드 실제 등록상표
   충돌) 등은 명시적으로 배제/보류. **실제 InDesign 라이브 검증 완료(사용자
   확인 "문제 없이 잘 작동해" — 컨텐츠/메세지/몇일 전부 정상 치환 제안).**
2. **Kiwi 라이선스 재평가(사용자 확인, 2026-08-27):** LGPL-3.0은 애초에 상업적/
   비공개소스 프로젝트에서도 쓰라고 만든 라이선스라 SmartLinter 자체를 오픈소스로
   풀 필요는 없음(Kiwi 자체 소스공개+저작권고지+교체가능성 조건만 지키면 됨).
   **게다가 SmartLinter는 상업 제품이 아니라 사내 전용 도구** — 이 두 가지가
   합쳐져서 서베이 때 Kiwi의 단점으로 꼽았던 "라이선스 검토 부담"이 사실상 거의
   사라짐(남는 비용은 순수 FFI/패키징 엔지니어링뿐). Kiwi의 상대적 매력도가
   서베이 당시보다 올라간 상태로 다음 조사 호응 스코핑에 반영할 것.
3. **조사 호응 설계에 사용자가 추가한 중요 원칙(나은/나는 사례, 아직 미구현):**
   "나은 누구인가"처럼 원문이 [고유명사 "나은"+조사 누락] 인지 [대명사
   "나"+조사 오류("는"→"은")] 인지 원천적으로 중의적인 경우가 있음 — 사용자가
   "이런 경우 후보(나은은 누구인가 / 나는 누구인가)를 둘 다 제시해야지 하나로
   억지로 고치면 안 된다"고 명시. 향후 조사 호응 스코핑(Kiwi 등)에 **QaIssue가
   지금은 `suggested_segment` 단일 문자열만 지원하는데, 이런 중의적 케이스는
   여러 후보 제안을 동시에 보여줘야 할 수도 있다**는 요구사항으로 필히 반영할
   것 — 스코핑 재조율 시작할 때 Codex+agy에게 이 스키마 확장 필요성부터
   물어볼 것.
2. **2단계(조사 호응 스코핑 재조율)는 다음 순서.** 원래 서베이에서 갈렸던
   "대명사 닫힌집합 화이트리스트를 Kiwi 없이 지금 배선해도 되는가"(agy: 가능
   / Codex: 형태소분석 확인 전엔 금지)를 재조율할 것. 상세는
   `AGY_ANSWER_LOCAL_MODEL_LINGUISTIC_QUALITY.md`/`CODEX_ANSWER_LOCAL_MODEL_LINGUISTIC_QUALITY.md`.
