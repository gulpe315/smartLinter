...
**마지막 업데이트: 2026-08-27 세션 종료 시점. 계획됐던 작업 전부 완료,
커밋 완료, 워킹트리 clean. 다음 세션 시작 시 아래 "다음 세션 우선순위"부터
읽을 것 — 재설계/재검토 불필요, 전부 실행 대기 상태.**

## 다음 세션 우선순위 (사용자가 둘 중 뭘 먼저 할지 정할 것 — 자동 결정 금지)

1. **실제 InDesign 라이브 확인 (미완료).** 조사 호응 화이트리스트(9스템/27매핑,
   커밋 `181d76b`)가 `detect()`에 연결됐지만, 이번 세션은 사용자 지시로
   라이브 검증을 계속 생략함 — 아직 한 번도 실제 InDesign에서 확인 안 됨.
   Rust 코드가 바뀌었으므로 브릿지 서버 재기동 필요(Claude가 직접 처리,
   사용자에겐 InDesign 재연결만 요청). 확인 절차: 27개 매핑 중 몇 개
   (예: "그들는", "우리을" 등)를 오탈자로 입력해 카드가 정상 뜨는지,
   교정 문구가 자연스러운지, 실제 적용이 되는지.
2. **Kiwi 통합 스파이크 (Part C, 착수 전).** 우선순위가 P0→P1/P2로
   하향됐고(Part A/B 완료로 급하지 않음, agy+Codex 둘 다 동의), 아직 코드
   착수 전. 시작한다면 첫 단계는 문법 규칙이 아니라 오프라인 패키징
   실현가능성 검증(`kiwi-rs` 버전 고정 → 네트워크 차단 Windows VM에서
   20회 콜드 기동). 상세는 `CODEX_ANSWER_BACKLOG_REVIEW_ROUND1.md`/
   `AGY_ANSWER_BACKLOG_REVIEW_ROUND1.md` Part C 참고.

이 둘 다 사용자가 명시적으로 요청하기 전엔 착수하지 말 것 — 이번 세션
끝에 "다음에 뭘 할지"를 사용자에게 물어보고 답 대기 중인 상태로 종료함.
