...
**별도로, 로컬 LLM 언어품질 서베이(조사 호응 등) — Codex+agy 답변 둘 다 도착,
종합해서 사용자에게 보고 완료, 다음 액션은 사용자 결정 대기 중.** 두 모델 다:
①현재 스코프 미비 아님(프롬프트가 이미 조사/문법을 요청 중, 이번 미탐지는
소형모델의 순수 탐지실패) ②받침 기반 조사(은/는/이/가/을/를 등)는 원리상
결정론적이지만 "먹는/좋은"류 용언 활용어미와 표면형이 겹쳐서 순진한 정규식은
위험 ③Kiwi(오프라인 한국어 형태소 분석기, C++/Rust 바인딩)가 안전한 개방어휘
조사검사의 유력한 인프라 후보 ④파인튜닝/LoRA·모델 교체·2-pass 전부 기각(각각
라이선스/스타일드리프트 재현 위험/VRAM 여유 부족/레이턴시 2배+큐 정체 악화)에
수렴. **유일한 불일치:** agy는 대명사(그들/우리/이것 등) 닫힌집합 화이트리스트는
Kiwi 없이도 즉시 100% 정밀도로 배선 가능하다는 입장(ROI #2), Codex는 그 경우도
형태소 분석 확인 없이는 아직 "Tier-1 취급 금지"라는 더 보수적 입장 — 재조율은
아직 안 함(구현 여부 자체가 미결정이라 우선순위 낮음). 상세는
