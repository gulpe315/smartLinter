...
Step 2는 환경 준비 이슈로 보류. 다음 세션 시작 시 아래 "다음 세션
우선순위"부터 읽을 것.**
...
1. **실제 InDesign 라이브 확인 (미완료).** 조사 호응 화이트리스트(9스템/27매핑,
   커밋 `181d76b`)가 `detect()`에 연결됐지만, 아직 한 번도 실제 InDesign에서
   확인 안 됨(2026-08-27 두 세션 연속 사용자 지시로 보류). Rust 코드가
   바뀌었으므로 브릿지 서버 재기동 필요(Claude가 직접 처리, 사용자에겐
   InDesign 재연결만 요청). 확인 절차: 27개 매핑 중 몇 개(예: "그들는",
   "우리을" 등)를 오탈자로 입력해 카드가 정상 뜨는지, 교정 문구가
   자연스러운지, 실제 적용이 되는지.
2. **Kiwi 스파이크 Step 2 환경 준비.** 위 참고 — Windows Sandbox 활성화
   또는 기존 VM 소프트웨어 중 사용자가 정할 것.

둘 다 사용자가 명시적으로 요청하기 전엔 착수하지 말 것.
